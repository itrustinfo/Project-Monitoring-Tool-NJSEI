                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        int Cnt = getdata.InsertorUpdateDocumentStatus(StatusUID, new Guid(Request.QueryString["DocID"].ToString()), 1, Status, 0, CDate1, DocPath,
                            new Guid(Session["UserUID"].ToString()), Comments, Status, txtrefNumber.Text, Document_Date, CoverPagePath, RBLDocumentStatusUpdate.SelectedValue, RBLOriginator.SelectedValue, docfiletobytes, coverfiletobytes);
                        if (Cnt > 0)
                        {
